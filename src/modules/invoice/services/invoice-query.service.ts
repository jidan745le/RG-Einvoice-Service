...
export class InvoiceQueryService {
    ...
    async findAll(queryDto: QueryInvoiceDto, tenantId?: string, authorization?: string): Promise<{
        ...
        const { fromEpicor = false, ...filters } = queryDto;

        try {
            // 如果明确要求从Epicor获取数据，或者有特定的实时查询需求
            if (fromEpicor || this.shouldQueryEpicorDirectly(queryDto)) {
                this.logger.log('Querying directly from Epicor based on request parameters');
                const result = await this.queryFromEpicorDirect(queryDto, tenantId, authorization);
                return { ...result, dataSource: 'epicor' };
            }
            ...
            if (!fromEpicor) {
                this.logger.warn('Cache query failed, falling back to Epicor direct query');
                try {
                    const result = await this.queryFromEpicorDirect(queryDto, tenantId, authorization);
                    return { ...result, dataSource: 'epicor' };
                } catch (epicorError) {
                    this.logger.error(`Epicor fallback also failed: ${epicorError.message}`);
                }
            }
